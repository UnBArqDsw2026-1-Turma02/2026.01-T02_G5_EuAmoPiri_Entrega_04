# Módulo 07 — Cadastro, Edição e Exclusão de Locais (Morador)
...
flowchart TB
    subgraph cliente [Cliente SPA]
        FE[CreatePlacePage / EditPlacePage]
    end

    subgraph middleware [Middleware reutilizado]
        Auth[authMiddleware]
        Morador[requireMorador]
        Multer[uploadPlacePhotos]
    end

    subgraph libs [Bibliotecas / serviços]
        Passport[Passport JWT]
        MulterLib[multer]
        GCS[storageService → GCS]
        Prisma[Prisma ORM]
    end

    subgraph app [Implementação própria]
        PlaceCtrl[placeController]
        PlaceSvc[placeService]
        PlaceModel[placeModel]
        PhotoVal[photoValidation]
    end

    FE -->|multipart POST/PATCH| Auth
    Auth --> Passport
    Auth --> Morador
    Morador --> Multer
    Multer --> MulterLib
...
- [Módulo 03 — Perfil e GCS](/ArquiteturaReutilizacao/backend/03.PerfilArmazenamento.md)
