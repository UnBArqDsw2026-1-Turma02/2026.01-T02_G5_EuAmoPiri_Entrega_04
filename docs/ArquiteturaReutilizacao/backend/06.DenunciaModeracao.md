# Módulo 06 — Denúncia e Moderação Comunitária (RF11)
...
## 4. O que a equipe implementou (não reutilizou)
...
| Ocultação automática por N denúncias | Decisão de projeto: revisão humana explícita |
| Denunciar `Place` | Fora do requisito — foco em UGC (relato/comentário) |

---

## 7. Rastreabilidade — RF11
...
| Ocultar automaticamente com **> 5 denúncias** | **Não implementado** | Ocultação apenas via admin (`PATCH /admin/experiences/:id/hide`). Relato `REPORTED` permanece visível publicamente até decisão humana |
...
- [Módulo 07 — Comentários](/docs/ArquiteturaReutilizacao/backend/07.ComentariosReacoes.md)
...
| 1.3 | 22/06/2026 | Remoção da § 4 (diagramas de ciclo de status e arquitetura); renumeração das seções |
