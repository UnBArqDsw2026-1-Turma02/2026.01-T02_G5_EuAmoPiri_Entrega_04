# Módulo 08 — Relatos de Experiência, Painéis e Validação de Conteúdo
...
```mermaid
sequenceDiagram
    participant Turista as Turista (RF09)
    participant Morador as Morador (RF10)
    participant API as Backend API
    participant DB as PostgreSQL

    Turista->>API: GET /auth/me/experiences (JWT)
    API->>DB: findExperiencesByUserId(userId)
    DB-->>API: relatos ordenados desc
    API-->>Turista: formatMyExperienceList

    Morador->>API: GET /places?moradorId={id}
    API->>DB: findPlacesByMoradorId
    DB-->>API: locais do morador
    loop Para cada local
        Morador->>API: GET /places/{id}/experiences
        API->>DB: findAllExperiencesByPlaceId
        DB-->>API: relatos públicos desc
        API-->>Morador: formatExperienceList
    end
```

A equipe **reutilizou endpoints existentes** em vez de criar `/morador/dashboard` — modificabilidade futura pode consolidar em um BFF agregador.
...
- [Módulo 07 — Locais Morador](/ArquiteturaReutilizacao/backend/07.LocaisMorador.md)
- [Módulo 04 — Comentários e reações](/ArquiteturaReutilizacao/backend/04.ComentariosReacoes.md)
- [Módulo 02 — Autenticação](/ArquiteturaReutilizacao/backend/02.Autenticacao.md)
