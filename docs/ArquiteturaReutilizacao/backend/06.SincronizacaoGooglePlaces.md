# Módulo 06 — Sincronização e Categorização Google Places (RF15)
...
sequenceDiagram
    ...
    participant Google as Places Photo API

    FE->>API: GET /places/43/cover
    API->>DB: lê externalPhotoUrl
    API->>Google: fetch media com GOOGLE_MAPS_API_KEY
    Google-->>API: stream JPEG/PNG
    API-->>FE: 200 image/jpeg Cache-Control public
```
...
## 6. Pipeline de sincronização

```mermaid
flowchart TD
    Start[startup ou POST sync admin] --> Fetch[fetchAllGooglePlacesFromApi]
    Fetch --> Q1[pousada em Pirenópolis GO]
    Fetch --> Q2[restaurante em Pirenópolis GO]
    Fetch --> Q3[cachoeira em Pirenópolis GO]
    Q1 --> Page[paginação pageToken max 40/query]
    Q2 --> Page
    Q3 --> Page
    Page --> Dedupe[dedupe por googlePlaceId]
    Dedupe --> Group[groupByCategory]
    Group --> Sort[ordena userRatingCount desc rating desc]
    Sort --> Slice[top GOOGLE_SYNC_PER_CATEGORY]
    Slice --> Upsert[upsertGoogleSyncedPlace Prisma]
```

---

## 7. Arquitetura no Eu Amo Piri

```mermaid
flowchart TB
    subgraph externo [Software externo reutilizado]
        GAPI[Google Places API New]
    end

    subgraph adaptadores [Adaptadores próprios]
        GPSvc[googlePlacesService]
        Mapper[placeCategoryMapper]
        SyncSvc[googlePlacesSyncService]
    end

    subgraph dominio [Domínio Eu Amo Piri]
        PlaceModel[placeModel]
        PlaceView[placeView]
        PlaceCtrl[placeController]
        CoverProxy["GET /places/:id/cover"]
    end
