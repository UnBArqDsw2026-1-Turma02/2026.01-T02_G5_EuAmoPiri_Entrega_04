# Módulo 09 — Consulta Pública de Locais e Relatos
...
```mermaid
flowchart LR
    subgraph publico [Visitante / Turista]
        FE[PlacesPage / PlaceDetailPage]
    end

    subgraph api [Backend — consulta]
        GETplaces[GET /places]
        GETexp[GET /places/:id/experiences]
        GETcover[GET /places/:id/cover]
    end

    subgraph reused [Reutilizado]
        Prisma[Prisma ORM]
        Views[placeView / experienceView]
        GCS[storageService]
        Google[googlePlacesService]
        OptAuth[optionalAuthMiddleware]
    end
...
Documentação de frontend: consumo em [rf04-cadastro-local](/requisitos/rf04-cadastro-local.md) (listagem pós-cadastro) e `PlacesPage.jsx`.

Integração Google: [Módulo 06 — Sincronização Google Places](/ArquiteturaReutilizacao/backend/06.SincronizacaoGooglePlaces.md).
...
- [Módulo 08 — Relatos](/ArquiteturaReutilizacao/backend/08.RelatosExperiencia.md)
- [Módulo 06 — Google Places](/ArquiteturaReutilizacao/backend/06.SincronizacaoGooglePlaces.md)
