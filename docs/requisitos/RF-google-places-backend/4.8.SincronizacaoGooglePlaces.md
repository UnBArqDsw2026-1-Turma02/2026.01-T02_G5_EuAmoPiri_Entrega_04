...
sequenceDiagram
    ...
    participant G as Places Photo API

    Note over FE,G: GET /places retorna coverImage "/places/43/cover"
    FE->>API: GET /places/43/cover
    API->>Svc: getPlaceCoverStream(43)
    Svc->>Svc: lê externalPhotoUrl no PostgreSQL
    Svc->>GPS: fetchExternalPhotoMedia(url)
    GPS->>G: GET media key=GOOGLE_MAPS_API_KEY
    G-->>GPS: stream imagem
    GPS-->>Svc: ReadableStream
    Svc-->>API: contentType + stream
    API-->>FE: 200 image/jpeg
```
...
Documentação complementar do padrão: [Módulo 06 — Proxy de capa](/docs/ArquiteturaReutilizacao/backend/06.SincronizacaoGooglePlaces.md#5-proxy-de-capa-google--padrao-estrutural-do-projeto).
