# 4.7. Denúncia e Moderação de Relatos e Comentários — Visão Arquitetural
...
erDiagram
    ...
    Experiences {
        int id PK
        enum status "ACTIVE | REPORTED | HIDDEN"
    }
    ...
    ExperienceReport {
        ...
        int experienceId FK
        int reporterId FK
        enum reason "ODIO | FALSO | SENSIVEL | OUTRO"
    }

    ExperienceCommentReport {
        int id PK
        int commentId FK
        int reporterId FK
        enum reason
...
flowchart TB
    ...
    subgraph controllers [Controllers]
        ERC[experienceReportController]
    ...
    subgraph models [Models]
        ERM[experienceReportModel]
        CRM[commentReportModel]
        EM[experienceModel]
        CM[commentModel]
    end
...
```mermaid
sequenceDiagram
    participant U as Turista_ou_Admin
    participant R as experienceRoutes
    participant S as experienceReportService
    participant DB as PostgreSQL

    U->>R: POST /experiences/:id/report { reason }
    R->>S: reportExperience
    S->>DB: INSERT ExperienceReport
    alt status ACTIVE
        S->>DB: UPDATE status REPORTED
    end
    S-->>U: 201 { message, reportCount, status }
```
...
sequenceDiagram
    ...
    A->>AR: GET /admin/moderation?status=REPORTED
    AR-->>A: fila de relatos e comentários

    A->>AR: PATCH /admin/experiences/:id/hide
    AR->>MS: hideExperience
    MS->>DB: status HIDDEN
    Note over DB: Item removido das listagens públicas
```
