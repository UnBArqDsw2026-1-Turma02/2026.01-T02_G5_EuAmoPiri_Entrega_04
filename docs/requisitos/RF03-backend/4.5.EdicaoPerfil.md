# 4.5. Foto de Perfil — Google Cloud Storage (Visão Arquitetural)

Documento técnico para o DAS e o módulo de Reutilização de Software do projeto **Eu Amo Piri**.
...
flowchart TB
  ...
  Routes --> Controller
  Controller --> ProfileSvc
  ProfileSvc --> UserEntity
...
flowchart LR
  ...
  subgraph fachada [api/auth]
    AA[authFacade]
  end
  subgraph modulos [api/auth]
    API[authApi]
    MAP[authMapper]
    SESS[authSessionStorage]
  end
  subgraph transporte [api]
    CL[client.js]
  end
  AC --> AA
  AA --> API
...
sequenceDiagram
  ...
  participant U as uploadProfilePhotoMiddleware
  participant PS as profileService
  participant SS as storageService
  participant DB as Postgres
  participant GCS as GCS_Bucket

  C->>R: PATCH /auth/me multipart
  R->>M: validar JWT
  M->>U: multer memoryStorage
  U->>PS: updateProfile
  PS->>DB: findById
  PS->>PS: hasProfileChanges
  PS->>SS: uploadBuffer
  SS->>GCS: save novo objeto
  PS->>DB: update profilePhotoUrl
  PS->>SS: deleteObject foto antiga
  PS-->>C: 200 user atualizado
```
