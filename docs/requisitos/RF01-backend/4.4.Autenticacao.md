...
O Eu Amo Piri é uma aplicação web para compartilhamento de experiências sobre Pirenópolis. Para liberar funcionalidades colaborativas (ex.: registrar experiências em locais), o sistema exige autenticação de usuários por:

- **Email e senha** (cadastro local)
- **Google OAuth** (login social)

Este módulo foi implementado no backend (`backend/`) reutilizando bibliotecas consolidadas do ecossistema Node.js, com frontend mínimo de teste em `frontend/` e documentação Swagger em `/api-docs`.
